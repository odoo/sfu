...
the SFU and a [client bundle/library](./doc/client.md) to connect to it.
...
interacted with as described [here](./doc/client.md).
...
The `resources`, `recordings` and `debug` directories under `DATA_PATH` must share
one filesystem mount. Recording finalization uses filesystem renames, which fail
with `EXDEV` across mounts.
...
| `RECORDING_RESERVATION_MB`               | Automatic worst case | No | Positive disk-space admission estimate per recording in MB (1 MB = 1,000,000 bytes). Overrides the estimate based on maximum duration, stream count and bitrates. |
...
`RECORDING_RESERVATION_MB` controls the free-space check when a recording starts.
It is an admission estimate, not a disk quota.

See [config.ts](./src/config.ts) for more details.
...
See [server.ts](./src/server.ts) for more details.
