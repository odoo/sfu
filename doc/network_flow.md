...
```json
{
  ...
  "user_id": 42, // optional authenticated Odoo user identity
  "label": "User Name",
  "permissions": {
    "recording": true,
...
2. Client sends credentials as first message:
   ...
   ```
3. The SFU acknowledges authentication with startup data. For a channel created
   without WebRTC, it returns:
   ```json
   {
     "availableFeatures": {
       "rtc": false,
...
The client also accepts a legacy empty acknowledgement and keeps its default
capabilities and recording state until later updates.
...
The client exchanges device information with the server
...
`SfuClient.setRecording` sends `CLIENT_REQUEST.SET_RECORDING` with optional
`audio`, `video` and `transcription` flags. Omitted flags preserve their current
values. The session validates permissions and active recording restrictions.
An empty resulting output set stops and finalizes the recording.

The boolean response acknowledges acceptance. `CH_INFO_CHANGE` carries the
resulting recording state and an optional stop code to every participant.

Finalized recordings use the [upload and completion contract](./recording.md#upload).
Odoo publishes media availability after the SFU confirms a successful upload.
