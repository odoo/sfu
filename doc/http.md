...
- GET `/v1/channel`: Creates or retrieves a channel for hosting calls
    ...
    - `webRTC` (string, optional): Defaults to `"true"`.
        - Set to `"false"` to create a signaling-only channel without WebRTC media capabilities.
          This can be useful for example when working in collaborative documents where audio/video are not necessary.
    - `recordingAddress` (string, optional):
        - If provided, enables recording for the channel.
        - Specifies an absolute HTTP(S) URL without embedded credentials for recording callbacks. Invalid URLs return `400 Bad Request` before channel creation.
