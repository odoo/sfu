...
export class MediaSink extends EventEmitter {
    ...
    private async _updateConsumer(available: boolean) {
        ...
        if (!available || !this._allowed) {
            this._stopListeningForFirstPacket();
        ...
        await this._listenForFirstPacket(consumer);
     ...
     */
    private async _listenForFirstPacket(consumer: Consumer) {
    ...
    private _stopListeningForFirstPacket() {
