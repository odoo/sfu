...
interface ChannelCreationClaims {
    /**
     * Issuer claim: Unique identifier for the channel creator, that can be unique per entry point, as it is used
    ...
    routeListener.get(`/v${API_VERSION}/channel`, {
        callback: async (req, res, { host, protocol, remoteAddress, searchParams }) => {
            try {
                ...
                const claims = auth.verify(jsonWebToken) as ChannelCreationClaims;
                if (!claims.iss) {
