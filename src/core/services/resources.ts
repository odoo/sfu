...
export const RECORDING_RESERVATION_BYTES = Math.ceil(
    config.RECORDING_RESERVATION_MB !== undefined
        ? config.RECORDING_RESERVATION_MB * 1_000_000
...
export class Folder {
    ...
    private static async _reserveMemory() {
        ...
        const stats = await fs.statfs(config.dir.resources, { bigint: true });
        const availableDiskBytes = stats.bsize * stats.bavail;
        const remaining = availableDiskBytes - BigInt(Folder._reservedRecordingBytes);
        if (remaining < BigInt(size)) {
            logger.warn(
                `block size: ${stats.bsize}, available blocks: ${stats.bavail}, available disk bytes: ${availableDiskBytes}`
            );
