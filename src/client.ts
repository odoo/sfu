...
import { Bus, type BusMessage } from "#src/shared/bus.ts";
import {
    CLIENT_MESSAGE,
    CLIENT_REQUEST,
    SERVER_MESSAGE,
...
import type { JSONSerializable, StreamType } from "#src/shared/types";
...
type DownloadStates = Partial<Record<StreamType, boolean>>;
...
export class SfuClient extends EventTarget {
        ...
        this._bus?.send(
            {
                ...
                payload: { info, needRefresh } as JSONSerializable
            },
            { batch: true }
        );
    }
    ...
    updateDownload(sessionId: SessionId, states: DownloadStates): void {
        ...
        for (const [type, active] of Object.entries(states)) {
            ...
            const consumer = consumers[type as keyof Consumers];
            if (consumer) {
        ...
        transport.on("produce", async ({ kind, rtpParameters, appData }, callback, errback) => {
            try {
                const result = (await this._bus!.request({
                    ...
                    payload: { type: appData.type, kind, rtpParameters } as JSONSerializable
                })) as { id: string };
    ...
    private async _handleMessage(message: JSONSerializable): Promise<void> {
        const { name, payload } = message as { name: string; payload?: ClientUpdatePayload };
        switch (name) {
    ...
    private async _handleRequest(request: JSONSerializable): Promise<JSONSerializable | void> {
        const { name, payload } = request as BusMessage;
        switch (name) {
            case SERVER_REQUEST.INIT_CONSUMER: {
                const { id, kind, producerId, rtpParameters, sessionId, type, active } =
                    payload as {
                        id: string;
                        kind: MediaKind;
                        producerId: string;
                ...
                if (!consumers) {
                    ...
                    consumers[type as keyof Consumers]?.close();
                }
                ...
                consumers[type as keyof Consumers] = consumer;
                return;
            }
            case SERVER_REQUEST.INIT_TRANSPORTS: {
                const { capabilities, stcConfig, ctsConfig, producerOptionsByKind } =
                    payload as unknown as {
                        capabilities: RtpCapabilities;
                        stcConfig: TransportConfig;
                        ctsConfig: TransportConfig;
                        producerOptionsByKind?: Record<string, ProducerOptions>;
                    };
