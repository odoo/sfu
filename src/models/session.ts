...
import type { Bus, BusMessage } from "#src/shared/bus.ts";
...
export class Session extends EventEmitter {
    ...
    private _broadcastInfo(): void {
        this._broadcast({
            ...
            payload: { [this.id]: this.info } as JSONSerializable
        });
    }
    ...
    private async _handleMessage(busMessage: JSONSerializable): Promise<void> {
        const { name, payload } = busMessage as BusMessage;
        switch (name) {
    ...
    private async _handleRequest(busMessage: JSONSerializable): Promise<JSONSerializable | void> {
        const { name, payload } = busMessage as BusMessage;
        switch (name) {
