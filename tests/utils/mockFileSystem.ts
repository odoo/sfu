...
export class MockFileSystem {
    ...
    ): { bsize: number | bigint; bavail: number | bigint } {
        const blockSize = 4096;
        const availableBlocks = Math.floor(this.availableDiskSpace / blockSize);
        return options?.bigint
            ? { bsize: BigInt(blockSize), bavail: BigInt(availableBlocks) }
            : { bsize: blockSize, bavail: availableBlocks };
    }
...
export const mockFsModule = {
    ...
    statfs: jest.fn((path: string, options?: StatFsOptions) =>
        Promise.resolve(mockFs.statfs(path, options))
    ),
